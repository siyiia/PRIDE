...
from reward_model_highlight import RewardHighlightModel
...
class Workspace(object):
    def __init__(self, cfg):
        ...
        self.reward_model = RewardHighlightModel(
    ...
    def run(self):
        ...
        while self.step < self.cfg.num_train_steps:
            ######################### diffusion model #################
            ###########################################################
            if (self.step + 1) % retrain_diffusion_step == 0 and (
                    self.step + 1) >= self.cfg.diffusion_start and self.step + 1 < self.cfg.num_train_steps and self.step > (
                    self.cfg.num_seed_steps + self.cfg.num_unsup_steps):
            ...
            if self.step == (self.cfg.num_seed_steps + self.cfg.num_unsup_steps):
                ...
                self.agent.update(self.replay_buffer, self.logger, self.step, 1, True, self.diffusion_replay_buffer,
                                  self.cfg.diffusion_sample_ratio)
...
@hydra.main(config_path='config/train_PEBwSyn_highlight.yaml', strict=True)
def main(cfg):
